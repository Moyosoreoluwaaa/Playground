...
@Composable
fun LangDraggableMatchNodeRight(
    word: LangMatchWord,
    isMatched: Boolean,
    modifier: Modifier = Modifier,
) {
    val matchedColor = MaterialTheme.colorScheme.primaryContainer
    ...
    val backgroundColor by animateColorAsState(
        targetValue = if (isMatched) matchedColor else defaultColor,
        label = "node_bg_animation_right"
    )
    val borderColor by animateColorAsState(
        targetValue = if (isMatched) matchedBorder else defaultBorder,
        label = "node_border_animation_right"
    )
